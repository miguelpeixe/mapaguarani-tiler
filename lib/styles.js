  ...
  getCartoCSS: function(table) {
    ...
    switch(table) {
      case 'core_indigenousland':
        ...
        cartocss += '#' + table + "[zoom>=14] { polygon-opacity: 0.4; text-fill: #fff; text-opacity: 0.8; text-name: [name]; text-face-name: 'DejaVu Sans Book'; text-halo-fill: #000; text-halo-radius: 1; text-placement-type: simple; text-placements: 'E,NE,SE,W,NW,SW'; text-size: 26; }";

        cartocss += '#' + table + "[zoom>=16] { text-size: 22; }";
